...
from -OSINT-CIPHER.localuseragent import ua
from -OSINT-CIPHER.instruments import TrioProgress
...
def import_submodules(package, recursive=True):
    """Get all the -OSINT-CIPHER submodules"""
...
def check_update():
    """Check and update -OSINT-CIPHER if not the last version"""
    check_version = httpx.get("https://pypi.org/pypi/-OSINT-CIPHER/json")
...
║                                                                              ║
║              EDUCATIONAL • PYTHON • TERMINAL EDITION                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
""")
    print('Youtube  : https://@OFFICIAL_cyber_satyam27')
    print('Github : https://github.com/OFFICIALcybersatyam27')
    print('For BTC Donations : 💖 Donations are never expected, but they're always appreciated. If you can't donate, no worries at all—your support, kindness, and encouragement mean the world to us. We happily accept payments from the heart. ❤️ ')
...
'keybase': 'keybase.io'}
    try:
        ...
        out.append({"name": name,"domain":data[name],
                    ...
                    "others": None})
async def maincore():
    parser= ArgumentParser(description=f"-OSINT-CIPHER v{__version__}")
    ...
    if not is_email(email):
        exit("[-] Please enter a target email ! \nExample : -OSINT-CIPHER email@example.com")

    # Import Modules
    modules = import_submodules("-OSINT-CIPHER.modules")
